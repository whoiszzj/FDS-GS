...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if not viewpoint_stack:
            viewpoint_stack = scene.getTrainCameras().copy()  # dict
        keys = list(viewpoint_stack.keys())
        viewpoint_cam = viewpoint_stack.pop(keys[randint(0, len(keys) - 1)])
...
def training_report(tb_writer, iteration, loss_dict, l1_loss, elapsed, testing_iterations, scene: Scene, renderFunc,
    ...
    if iteration in testing_iterations or (show_debug_info and iteration % 500 == 0):
        ...
        train_keys = list(scene.getTrainCameras().keys())
        validation_configs = (
            {'name': 'test', 'cameras': list(scene.getTestCameras().values())},
            {
                'name': 'train',
                'cameras': [
                    scene.getTrainCameras()[train_keys[idx % len(scene.getTrainCameras())]] for idx in range(5, 30, 5)
                ]
            }
        )
