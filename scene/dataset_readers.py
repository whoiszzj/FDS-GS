...
class CameraInfo(NamedTuple):
    ...
    width: int
    height: int
    original_intrinsics: np.array = None
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        original_intrinsics = np.array(
            [[intr.params[0], 0, intr.params[2]], [0, intr.params[1], intr.params[3]], [0, 0, 1]])

        cam_info = CameraInfo(
            uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
            image_path=str(image_path), image_name=image_name, width=width, height=height,
        ...
        for idx, frame in enumerate(frames):
            ...
            cam_infos.append(
                CameraInfo(
                    ...
                    image_path=str(image_path), image_name=image_name, width=image.size[0],
                    height=image.size[1]
                )
            )
